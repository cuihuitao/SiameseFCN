...
switch opts.branch.type
    ...
    case 'test3'
        [branch1, branch2] = make_branches_test3(opts);
    case 'resnet_block1'
        [branch1, branch2] = make_branches_resnet_block1(opts);
    case 'matvggm5'        
...
function [branch1, branch2] = make_branches_test3(opts)
    branch_opts.last_layer = 'merge3';
    branch_opts.num_out     = [64, 128, 256];
    branch_opts.num_in      = [ 3,  64, 128];
    branch_opts.conv_stride = [ 1,   1,   1];
    branch_opts.pool_stride = [ 2,   1];
    branch_opts.batchNormalization = true;
    branch_opts = vl_argparse(branch_opts, {opts.branch.conf});
    ...
    f = @() make_branch_resnet3(branch_opts);
    branch1 = f();
    branch2 = f();
    
end

function [branch1, branch2] = make_branches_resnet_block1(opts)
    branch_opts.last_layer = 'conv2_1_sum';   
    branch_opts = vl_argparse(branch_opts, {opts.branch.conf});
    branch_opts.exemplarSize = opts.exemplarSize * [1 1];
    branch_opts.instanceSize = opts.instanceSize * [1 1];
    
    f = @() make_branch_resnet_block1(branch_opts);
    branch1 = f();
    branch2 = f();   
end

function [branch1, branch2] = make_branches_matvggm5(opts)
    branch_opts.last_layer = 'conv5_conv';   
    branch_opts = vl_argparse(branch_opts, {opts.branch.conf});
    branch_opts.exemplarSize = opts.exemplarSize * [1 1];
    branch_opts.instanceSize = opts.instanceSize * [1 1];
    
    f = @() make_branch_matvggm5(branch_opts);
    branch1 = f();
    branch2 = f();   
end

function [branch1, branch2] = make_branches_matvggf5(opts)
    branch_opts.last_layer = 'conv5_conv';   
    branch_opts = vl_argparse(branch_opts, {opts.branch.conf});
    branch_opts.exemplarSize = opts.exemplarSize * [1 1];
    branch_opts.instanceSize = opts.instanceSize * [1 1];
    
    f = @() make_branch_matvggf5(branch_opts);
    branch1 = f();
    branch2 = f();   
end
...
function [branch1, branch2] = make_branches_matvgg16(opts)
    ...
    f = @() make_branch_matvgg16(branch_opts);
    branch1 = f();
    branch2 = f();   
end

function [branch1, branch2] = make_branches_darknet19(opts)
    branch_opts.last_layer = 'conv6_5_conv';   
    branch_opts = vl_argparse(branch_opts, {opts.branch.conf});
    branch_opts.exemplarSize = opts.exemplarSize * [1 1];
    branch_opts.instanceSize = opts.instanceSize * [1 1];
